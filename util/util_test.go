...
// define the network and bit manipulation helpers
...
func TestBitsOperation(t *testing.T) {
	testBytes := []byte{0xb7}

	if res1 := TestAndSet(testBytes); res1 != 4 {
		t.Fatal("TestAndSet1 failed")
	}

	if res2 := TestAndSet(testBytes); res2 != 7 {
		t.Fatal("TestAndSet2 failed")
	}
	if res3 := TestAndSet(testBytes); res3 != 9 {
		t.Fatal("TestAndSet3 failed")
	}
}
